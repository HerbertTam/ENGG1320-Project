        ...
        if len(frames) == 128:
            ...
            for frame_number_in_batch, face_locations in enumerate(batch_of_face_locations):
                ...
                for face_location in face_locations:
                    ...
                    #crop_img = frames[frame_number_in_batch][bottom:top, left:right]
                    if known:
                        name = 'known_people/{}_{}.jpg'.format(round(frame_number/fps),count)
                    else:
                        name = 'unknown_pictures/{}_{}.jpg'.format(round(frame_number/fps),count)
                    cv2.imwrite(name, frames[frame_number_in_batch])
                    if known:
...
def recognitionFace(unknown_pictures,known_people):
    for unknown_picture in unknown_pictures: #check the faces in the image one by one
...
def addData(image):
  global imageBase
  imageBase.append([image, time.time()])
...
def main():
    ...
    imageIndex = 0
    duration = 0
    findFacesInVideo("in1.mp4", True)
    findFacesInVideo("out1.mp4", False)
    flag, image = recognitionFace("unknown_pictures","known_people")
    if (flag == True):
